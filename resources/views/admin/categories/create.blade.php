<x-layouts.app :title="__('Create Category')">
    <div class="p-6 max-w-4xl mx-auto">
        <header class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Create New Category</h1>
            <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Add a new category to organize your services</p>
        </header>
        ...
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg">
            <form action="{{ route('admin.categories.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Leave empty to auto-generate from title</p>
                    @error('slug')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" 
                        id="description" 
                        rows="4"
                        class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
