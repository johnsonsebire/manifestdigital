    ...
    <div class="p-6 max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex items-center space-x-2 text-sm text-zinc-600 dark:text-zinc-400 mb-2">
                ...
                <span>/</span>
                <a href="{{ route('admin.services.show', $service) }}" class="hover:text-primary-600" wire:navigate>{{ $service->title }}</a>
                <span>/</span>
                <span>Edit</span>
            </div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Edit Service</h1>
        </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow p-6 space-y-4">
                ...
                <div>
                    ...
                    <input 
                        ...
                        class="w-full px-4 py-2 border border-zinc-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 @error('slug') border-red-500 @enderror"
                    >
                    @error('slug')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Leave blank to auto-generate from title</p>
                </div>
                ...
                <div>
                    ...
                    >{{ old('description', $service->long_description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="subscription" {{ old('type', $service->type) === 'subscription' ? 'selected' : '' }}>Subscription</option>
                            <option value="custom" {{ old('type', $service->type) === 'custom' ? 'selected' : '' }}>Custom</option>
                            <option value="one_time" {{ old('type', $service->type) === 'one_time' ? 'selected' : '' }}>One Time</option>
                            <option value="ai_enhanced" {{ old('type', $service->type) === 'ai_enhanced' ? 'selected' : '' }}>AI Enhanced</option>
                            <option value="consulting" {{ old('type', $service->type) === 'consulting' ? 'selected' : '' }}>Consulting</option>
                            <option value="add_on" {{ old('type', $service->type) === 'add_on' ? 'selected' : '' }}>Add-on</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="base_price" 
                            name="base_price"
                            value="{{ old('base_price', $service->price) }}"
                            step="0.01"
                            min="0"
                    ...
                    <div>
                        <label class="flex items-center space-x-2">
                            <input 
                                ...
                                name="is_available" 
                                value="1"
                                {{ old('is_available', $service->available) ? 'checked' : '' }}
                                class="rounded border-zinc-300 text-primary-600 focus:ring-primary-500"
                            >
                    ...
                    <div>
                        <label class="flex items-center space-x-2">
                            <input 
                                ...
                                name="is_visible" 
                                value="1"
                                {{ old('is_visible', $service->visible) ? 'checked' : '' }}
                                class="rounded border-zinc-300 text-primary-600 focus:ring-primary-500"
                            >
